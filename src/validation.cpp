...
#if defined(NDEBUG)
# error "Spore cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Spore Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s SPR additional fees, %d delta bytes\n",
